    ...
    def cmd(cmd_wrd):
        ...
        def repeatCb(timestamp):
            while True:
                ...
                timestamp=rospy.get_param('advanced_recognizer/time')
        ...
        if cmd_wrd[0].find("stop") > -1:
            ...
            print("stopped")
        if cmd_wrd[0].find("shift") > -1 and cmd_wrd[1].find("lower") > -1:
            print("left joints")
            for i in range(0,6):
                print(left.joint_velocity(lj[i]))
            print("right joints")
            for i in range(0,6):
                print(right.joint_velocity(rj[i]))
            ...
            if cmd_wrd[0].find("left") > -1 or cmd_wrd[0].find("right") > -1:
                ...
                try: joint
                    ...
                    try: joint_dof
                        ...
                        #rospy.Subscriber('advanced_recognizer/timeoutput', Float64, repeatCb)
                        repeatCb(rospy.get_param('advanced_recognizer/time')) 
                        '''if side == "left":
                            set_j(left, joint_dof, velocity)
                            print(side)
                            print(joint_dof)
